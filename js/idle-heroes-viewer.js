...
class IdleHeroesViewer {
    ...
    renderSummary() {
        ...
        // Get user data
        const user = this.data.user || {};
        const heroCount = (this.data.heroes || []).length;
        const inventoryCount = (this.data.inventory || []).length;
    ...
    loadDemoData() {
        try {
            ...
            // Fallback if demo data couldn't be loaded
            console.error('Failed to load demo data');
            this.showError('Could not load demo data. Please check your installation.');
        } catch (error) {
    ...
    /**
     * Show a demo mode indicator to let users know they're viewing sample data
     */
    showDemoMode() {
     ...
     */
    async checkLocalCache(accountId) {
        try {
            // Check IndexedDB first (if available)
            if (window.DBManager) {
                const data = await DBManager.loadAccountData(accountId);
                if (data) {
                    return data;
                }
            }
